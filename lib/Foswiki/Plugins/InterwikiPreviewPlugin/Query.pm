...
sub new {
  ...
  if (exists $queries->{$queryid}) {
    Foswiki::Func::writeDebug( "- ${pluginName}::Query::new reusing '$queryid')" ) if $debug;
    return $queries->{$queryid};
  }

  Foswiki::Func::writeDebug( "- ${pluginName}::Query::new($queryid)" ) if $debug;
  ...
  Foswiki::Func::writeDebug( "- ${pluginName}::Query::new(${queryid}) cachecontrol=${cachecontrol} extractable=${extractable}" . ((defined $this->{cache}) ? " cached" : "") ) if $debug;
...
sub field {
  ...
  if ( exists $params{"source"} ) {
    ...
    my $next_field = Foswiki::Func::getSessionValue($pluginName.'NextField');
    my $field_id = "iwppf${next_field}";
    Foswiki::Func::setSessionValue($pluginName.'NextField',$next_field+1);
    ...
    if ( exists $this->{cache} ) {
      ...
      if ( exists $extracted{$params{"source"}} ) {
        ...
        Foswiki::Func::writeDebug( "- ${pluginName}::Query::field '${filler}' extracted from cache" ) if $debug;
      } else {
...
sub script {
  ...
  my $url = Foswiki::Func::getScriptUrl($pluginName,
    $this->{"rule"}->{"alias"},
    'rest',
    page => $this->{"page"});
...
sub scripts {
  ...
  foreach (values %{Foswiki::Func::getSessionValue($pluginName.'Queries')}) {
    $text = $text . $_->script();
  }
