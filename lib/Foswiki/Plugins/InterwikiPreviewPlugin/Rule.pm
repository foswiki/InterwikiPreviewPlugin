...
sub new {
  ...
  my $cache = new Cache::FileCache( { 'cache_root' => Foswiki::Func::getWorkArea( $pluginName )."/cache",
      'directory_umask' => '022',
      'namespace' => $alias } );
  ...
  Foswiki::Func::getSessionValue($pluginName.'Rules')->{$alias} = bless( $this, $class );
...
sub restHandler {
  ...
  Foswiki::Func::writeDebug( "- ${pluginName}::Rule::restHandler($subject,$verb,$page)" ) if $debug;

  my $query = Foswiki::Func::getRequestObject();
  return unless $query;

  my $httpCacheControl = $Foswiki::cfg{Plugins}{InterwikiPreviewPlugin}{HttpCacheControl};
  ...
  unless ( $httpCacheControl &&
    ...
    if ( defined $text ) {
      if ( $debug ) {
        ...
        Foswiki::Func::writeDebug( "- ${pluginName}::Rule::restHandler ${page} cached for ${expiry}s" );
      }
      $text =~ s/^(.*?\n)\n(.*)/$2/s;
      if( $1 =~ /content\-type\:\s*([^\n]*)/ois ) {
  ...
  if( $response->is_error() ) {
    ...
    $msg =~ s/[\n\r]/ /gos;
    Foswiki::Func::writeDebug( "- ${pluginName}::Rule ERROR: Can't read $url ($msg)" ) if $debug;
  ...
  if ( $expiry == 0 ) {
    $expiry = $Foswiki::Cfg{Plugins}{InterwikiPreviewPlugin}{DefaultCacheExpiry};
  }
  $text =~ s/\r\n/\n/gos;
  $text =~ s/\r/\n/gos;
